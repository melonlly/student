	...
	<div class="list">
		<div class="table_box">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr v-for="tr in data">
					<td v-for="(value, key) in tr" v-if="!columns.find(item => item.name === key).hide">
						{{columns.find(item => item.name === key).enum ? enumToValue(columns.find(item => item.name === key).enum, value) : value}}
					</td>
					<td v-if="hasOperate()">
						<a class="icon_edit" v-if="operate('edit')" @click="doIt('edit', tr)"></a>
